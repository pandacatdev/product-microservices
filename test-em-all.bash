...
#  body="{\"productId\":213,\"name\":\"product name B\",\"weight\":200,\"recommendations\":[{\"recommendationId\":1,\"author\":\"author 1\",\"rate\":1,\"content\":\"content 1\"},{\"recommendationId\":2,\"author\":\"author 2\",\"rate\":2,\"content\":\"content 2\"},{\"recommendationId\":3,\"author\":\"author 3\",\"rate\":3,\"content\":\"content 3\"}]}"
...
function testCompositeCreated() {
    ...
    # -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
    if ! assertCurl 200 "curl $AUTH https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS -s -k"
    then
        echo -n "FAIL"
        return 1
    fi
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 200 "curl -H "accept:application/json" https://u:p@$HOST:$PORT/eureka/api/apps -s -k"
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 200 "curl $AUTH https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS -s -k"
assertEqual $PROD_ID_REVS_RECS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 3 $(echo $RESPONSE | jq ".reviews | length")
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 404 "curl $AUTH https://$HOST:$PORT/product-composite/$PROD_ID_NOT_FOUND -s -k"
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 200 "curl $AUTH https://$HOST:$PORT/product-composite/$PROD_ID_NO_RECS -s -k"
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 200 "curl $AUTH https://$HOST:$PORT/product-composite/$PROD_ID_NO_REVS -s -k"
...
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 422 "curl $AUTH https://$HOST:$PORT/product-composite/-1 -s -k"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
# -k flag to support https self-signed cert (skip verify that certificate must be from trusted Certificate Authorities)
assertCurl 400 "curl $AUTH https://$HOST:$PORT/product-composite/invalidProductId -s -k"
